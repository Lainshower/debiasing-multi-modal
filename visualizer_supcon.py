...
class CustomCLIP(nn.Module):
    def __init__(self, adapter, text_embedding_dir, text_spurious_embedding_dir, temperature=0.01, head_method = None, ca_feat_dim = 128):
        super().__init__()
    ...
    def forward(self, features): 
        ...
        text_features = self.text_features / self.text_features.norm(dim=-1, keepdim=True) # (Pre) Normalized (B, 2, 1024)
...
def set_model(opt):
    ...
    if opt.tl_method =='linear_probing':
        ...
        adapter = Adapter(input_dim = input_dim, hidden_dim = opt.adapter_feat_dim) # Fixed by heuristics
        classifier = CustomCLIP(adapter, opt.text_embedding_dir, opt.text_spurious_embedding_dir, temperature=opt.zs_temperature, head_method=opt.ca_head, ca_feat_dim=opt.ca_feat_dim)
...
def train_one_epoch_cl(opt, train_loader, classifier, contrastive_loss, optimizer, epoch, print_label='Train'):
    ...
    print(f"Averaged Pos-term in {print_label}:", str(poss.avg))
    print(f"Averaged Neg-term in {print_label}:", str(negs.avg))
